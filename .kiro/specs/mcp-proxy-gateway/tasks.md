...
- [x] 13. 实现对外 MCP API 服务（MCP_API_Service）
  ...
  - [x] 13.5 编写对外三种传输端到端集成测试
    - SSE/HTTP/WS 各验证列表、搜索、批量获取定义与调用
    - _Requirements: 11.8_

  - [x] 13.6 增强智能模式工具检索与大工具集发现体验
    - 使用确定性本地分词、缩写展开与字段加权排序；支持搜索分页、零结果建议、来源消歧、上游概览筛选和批量获取工具定义；对输入、上游描述和 API Key 隔离短缓存施加固定边界，索引惰性缓存以保证稳定低延迟
    - _Requirements: 11.3, 11.4, 11.5, 11.7, 11.10_
