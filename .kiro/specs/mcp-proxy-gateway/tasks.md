...
- [x] 15. 实现认证服务（Auth_Service）
  - [x] 15.1 实现首次初始化、注册、登录、会话与改密
    - 首次无管理员进入初始化提供注册入口；注册校验用户名 3-32、密码 8-128，bcrypt 加盐哈希写入 YAML，已存在则拒绝保持单用户；登录匹配则签发有效期=会话超时（默认 3600s，300-86400s）的 JWT，不匹配拒绝；改密校验当前密码与新密码长度后更新哈希
...
- [x] 26. 实现前端各管理页面
  ...
  - [x] 26.2 实现规则管理页（别名/描述重写、屏蔽过滤）
    - 以 Tailwind CSS 工具类、遵循 TailAdmin 组件与风格（表格、表单、模态框）实现别名规则与 MCP 级屏蔽规则的增删改、启停、排序；大屏借助 Tailwind 响应式工具类启用多栏布局提升信息密度
