...
async function onInstall(pkg: RuntimeCatalogPackage): Promise<void> {
  ...
  try {
    ...
    toast.success(result.reused ? `${result.name} 已存在，已跳过下载` : `${result.name} ${result.version} 安装完成`)
    await load()
  } catch (err) {
    ...
    <template v-else-if="summary !== null">
      <div class="mb-5 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <section
          class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm transition duration-300 hover:shadow-md dark:border-gray-800 dark:bg-white/[0.03]"
        >
          <p class="text-xs text-gray-400 dark:text-gray-500">进程加固</p>
          <p class="mt-2 text-sm font-semibold text-gray-800 dark:text-white/90">{{ hardeningLabel }}</p>
          <p class="mt-2 text-xs leading-5 text-gray-500 dark:text-gray-400">
      ...
      >
        ...
        <div v-if="catalog.length === 0" class="rounded-xl border border-dashed border-gray-200 px-4 py-8 text-center text-sm text-gray-400 dark:border-gray-700">
          暂无可用预置包
        </div>
        ...
        <div v-else class="grid grid-cols-1 gap-3 lg:grid-cols-2">
          <article
            ...
            :key="pkg.id"
            class="group rounded-2xl border border-gray-100 bg-gradient-to-br from-gray-50/90 to-white p-4 transition duration-300 hover:border-brand-200 hover:shadow-md dark:border-gray-800 dark:from-white/[0.04] dark:to-white/[0.02] dark:hover:border-brand-500/30"
          >
            <div class="flex flex-wrap items-start justify-between gap-2">
              <div class="min-w-0">
                <h4 class="text-sm font-semibold text-gray-800 dark:text-white/90">
                  {{ pkg.name }}
                  <span class="ml-1 font-mono text-xs font-normal text-gray-400">{{ pkg.version }}</span>
                </h4>
            ...
            <div class="mt-4 flex flex-wrap gap-2">
              <button
                v-if="!pkg.installed"
                type="button"
                class="inline-flex h-9 items-center rounded-lg bg-brand-500 px-3.5 text-sm font-medium text-white transition hover:bg-brand-600 disabled:cursor-not-allowed disabled:opacity-50"
                :disabled="!pkg.supported || busyPackageId !== ''"
                :aria-label="`安装 ${pkg.name}`"
                @click="onInstall(pkg)"
              >
                <span v-if="busyPackageId === pkg.id" class="mr-1.5 inline-block h-3.5 w-3.5 animate-spin rounded-full border-2 border-white/40 border-t-white" />
      ...
      >
        <div class="mb-4 flex flex-wrap items-center justify-between gap-2">
          ...
          <div class="text-right text-xs text-gray-400 dark:text-gray-500">
            <p v-if="summary.pathPrefixes?.length">PATH 前缀 {{ summary.pathPrefixes.length }} 项</p>
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <article
            ...
            :key="tool.name"
            class="rounded-xl border border-gray-100 bg-gray-50/80 p-4 transition duration-300 hover:border-brand-200 hover:bg-white dark:border-gray-800 dark:bg-white/[0.03] dark:hover:border-brand-500/30"
          >
