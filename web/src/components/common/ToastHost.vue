...
const iconPath: Record<ToastType, string> = {
  ...
  warning:
    'M10 5.6 3.6 16.5h12.8L10 5.6Zm0 3.2c.42 0 .76.34.76.76v3.18a.76.76 0 0 1-1.52 0V9.56c0-.42.34-.76.76-.76Zm0 6.9a.9.9 0 1 1 0-1.8.9.9 0 0 1 0 1.8Z',
  error:
    'm10 8.82 3.18-3.18 1.18 1.18L11.18 10l3.18 3.18-1.18 1.18L10 11.18l-3.18 3.18-1.18-1.18L8.82 10 5.64 6.82l1.18-1.18L10 8.82Z',
  ...
  >
    <transition name="toast">
      <div
        v-if="item !== null"
        :key="item.id"
        class="shadow-theme-lg inline-flex min-h-12 max-w-[min(92vw,34rem)] items-center gap-2.5 rounded-lg border border-gray-100 bg-white px-5 py-3 text-sm font-medium text-gray-800 dark:border-gray-800 dark:bg-gray-900 dark:text-white/90"
        role="status"
      >
        <span
        ...
        <span class="min-w-0 leading-5 break-words">{{ item.message }}</span>
      </div>
    </transition>
  </div>
</template>
