...
// runRespondError 在一个最小 gin 上下文中调用 respondError，并返回状态码与解析后的错误体。
func runRespondError(t *testing.T, err error) (int, domain.APIError) {
	t.Helper()
...
// TestCodeToStatusMappingComplete 验证每个领域错误码都映射到约定的 HTTP 状态码。
func TestCodeToStatusMappingComplete(t *testing.T) {
	cases := []struct {
		...
		{domain.CodeUpstreamUnavailable, http.StatusBadGateway},
		{domain.CodeUpstreamTimeout, http.StatusGatewayTimeout},
		{domain.CodeToolNotFound, http.StatusNotFound},
		{domain.CodeBackupInvalid, http.StatusBadRequest},
	}
	for _, tc := range cases {
		t.Run(string(tc.code), func(t *testing.T) {
			...
			if body.Code != tc.code {
				t.Errorf("响应体错误码期望 %s，实际 %s", tc.code, body.Code)
			}
...
// TestValidationErrorCarriesPerFieldMap 验证校验错误响应携带逐字段的 Fields 映射（Req 2.2、4.8、14.10）。
func TestValidationErrorCarriesPerFieldMap(t *testing.T) {
	...
	if body.Code != domain.CodeValidation {
		t.Errorf("期望错误码 VALIDATION，实际 %s", body.Code)
	}
	if len(body.Fields) != len(fields) {
		t.Fatalf("期望 %d 个字段级错误，实际 %d：%+v", len(fields), len(body.Fields), body.Fields)
	}
	for k, v := range fields {
		if body.Fields[k] != v {
			t.Errorf("字段 %q 期望错误说明 %q，实际 %q", k, v, body.Fields[k])
		}
	}
}
...
func TestUnknownErrorMapsTo500WithoutLeak(t *testing.T) {
	...
	if body.Code != domain.CodeInternal {
		t.Errorf("期望兜底错误码 INTERNAL，实际 %s", body.Code)
	}
	if body.Message == secret {
		t.Error("响应不应回显原始错误信息（泄露内部细节）")
	}
	if body.Fields != nil {
		t.Errorf("通用 500 错误不应携带字段级信息，实际 %+v", body.Fields)
	}
}
...
func TestWrappedAPIErrorIsUnwrapped(t *testing.T) {
	...
	if body.Code != domain.CodeConflict {
		t.Errorf("期望错误码 CONFLICT，实际 %s", body.Code)
	}
}
...
func TestBindJSONInvalidBodyReturnsValidationError(t *testing.T) {
	...
	if err := json.Unmarshal(w.Body.Bytes(), &body); err != nil {
		t.Fatalf("解析错误响应体失败：%v", err)
	}
	if body.Code != domain.CodeValidation {
		t.Errorf("期望错误码 VALIDATION，实际 %s", body.Code)
	}
	if _, ok := body.Fields["body"]; !ok {
		t.Errorf("校验错误应通过 Fields 标识无效字段，实际 %+v", body.Fields)
	}
}
