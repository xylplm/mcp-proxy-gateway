...
func (a *Authenticator) Authenticate(ctx context.Context, plaintext string, now time.Time) (Metadata, bool, error) {
	...
	if plaintext == "" {
		return Metadata{}, false, domain.NewError(domain.CodeUnauthorized, "缺少 API Key")
	}
	...
	if err != nil {
		...
		return Metadata{}, false, domain.NewError(domain.CodeUnauthorized, "API Key 无效")
	}

	meta := toMetadata(row)
	if !meta.Usable(now) {
		// 已停用或已超过有效期（Req 12.4、12.5、12.6）。
		return Metadata{}, false, domain.NewError(domain.CodeUnauthorized, "API Key 已停用或已过期")
	}
...
func (a *Authenticator) Middleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		meta, ok, _ := a.Authenticate(c.Request.Context(), plaintext, time.Now())
		if !ok {
